...
**LE COMPTE DE LA SEMAINE TRANCHE.** Huit instruments faux entre le 31 août et
le 5 septembre : tous posés avec un témoin positif ou aucun, **aucun avec un
négatif**. Sept avaient été comptés le 5 septembre ; le huitième est la
quatrième forme ci-dessus — l'`Age` relevé par `curl` sur un nœud Fastly que
le navigateur n'avait pas atteint —, mesurée le 1er septembre et versée ici
après coup. Un compte daté d'une semaine se relit donc à chaque ajout : celui
d'avant n'était pas faux, il était complet pour ce qui était écrit ce jour-là.

Les deux qui ont menti le plus longtemps sont précisément ceux dont le positif
passait — le balayage à 99 %, et le diff d'index porté sur `t` plutôt que sur
`k`, où « aucun terme ajouté » était plausible et faux.
